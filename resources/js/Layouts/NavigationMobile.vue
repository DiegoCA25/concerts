      ...
      <div class="py-4 text-white">
        ...
        <li class="relative px-6 py-3">
          <button @click="showingTwoLevelMenu = !showingTwoLevelMenu"
            ...
            <ul v-show="showingTwoLevelMenu || route().current('concerts.index') || route().current('concerts.create') " class="p-2 mt-2 space-y-2 overflow-hidden text-sm font-medium text-gray-500 rounded-md shadow-inner bg-gray-50"
                aria-label="submenu">
              <li class="px-2 py-1 transition-colors duration-150 hover:text-gray-800">
                <ResponsiveNavLink :href="route('concerts.index')" :active="route().current('concerts.index')">
                  List
                </ResponsiveNavLink>
              </li>
              <li class="px-2 py-1 transition-colors duration-150 hover:text-gray-800">
                <ResponsiveNavLink :href="route('concerts.create')" :active="route().current('concerts.create')">
                  Create
                </ResponsiveNavLink>
              </li>
            </ul>
